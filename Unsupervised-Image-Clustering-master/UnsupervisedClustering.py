...
#from applications.nasnet import NASNetMobile
from keras import preprocessing
from PIL import Image as pimg
from keras_models.models import Model
...
def nasnet_model():
    base_model = applications.nasnet.NASNetMobile(weights='imagenet', include_top=True)
    ...
    for img_path in files:
        if(file_size(img_path)!=0):
            ...
            img = preprocessing.image.load_img("./images/"+str(img_path), target_size=(224, 224))
            # extract features from each image
            x_image = preprocessing.image.img_to_array(img)
            x_image = np.expand_dims(x_image, axis=0) # increase dimensions of x to make it suitable for further feature extraction
            x_image = applications.nasnet.preprocess_input(x_image)
